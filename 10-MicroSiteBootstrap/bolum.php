...
function bolumDizisiGetir($bolumNo)
{
    ...
    global $connection;
    $bolumQuery = $connection->prepare("SELECT * FROM bolumler WHERE bolumNo = :bolumNo");
    $bolumVarMi = $bolumQuery->execute(array(
        "bolumNo"		=>	$bolumNo,
    ));
    $bolum = $bolumQuery->fetch();
    return $bolum;
}
...
function sonBolumNumarasiniGetir()
{
    ...
    global $connection;
    $bolum = $connection->query("SELECT bolumNo FROM bolumler ORDER BY bolumNo DESC LIMIT 0,1")->fetch();
    return $bolum['bolumNo'];
}
